...
df=df.drop(columns='fruits‚_légumes_et_noix_-_séchés')
df.head()
...
# 1. Sélection du modèle champion (Random Forest)
rf_model_retenu = trained_models['Random Forest']

# 2. Obtenir les probabilités de prédiction pour toutes les classes
y_pred_proba = rf_model_retenu.predict_proba(X_test)

# 3. Binariser les vraies étiquettes (y_test) pour la stratégie One-vs-Rest
...
# Définissez les modèles de base à "stacker" (empiler)
base_estimators = [
    # Random Forest (Votre champion actuel)
    ('rf', RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)),

    # XGBoost (Un modèle par boosting, pour la diversité)
    ('xgb', xgb.XGBClassifier(use_label_encoder=False, eval_metric='mlogloss', random_state=42)),

    # K-Nearest Neighbors (KNN, un modèle de nature différente)
    ('knn', KNeighborsClassifier(n_neighbors=5, n_jobs=-1))
]

# L'estimateur final (méta-modèle) prend les prédictions des modèles de base
# comme input pour faire la prédiction finale.
...
stack_model = StackingClassifier(
    ...
    n_jobs=-1 # Exécution parallèle des modèles de base
)

print("🧠 Entraînement du Stacking Classifier...")
# L'entraînement inclut l'entraînement des modèles de base et du méta-modèle.
...
print(f"🚀 Précision du STACKING CLASSIFIER sur le jeu de test : {accuracy:.4f}")

# Comparaison avec les modèles de base individuels (pour s'assurer de l'amélioration)
print("\nComparaison avec les modèles de base :")
for name, model in base_estimators:
    # Ré-entraînement nécessaire car Stacking utilise des versions clonées/prétrainées
    model.fit(X_train, y_train)
